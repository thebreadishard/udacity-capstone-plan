...
4. **MP2 Is Not Good Enough: Transfer Learning ML Models for Accurate VPT2 Frequencies** (Käser, Boittier, Upadhyay & Meuwly, *JCTC*, 2021)
   ...
   - *Attribution corrected 2026-08-23:* this entry previously read *"Transfer Learning to CCSD(T): Accurate Anharmonic Frequencies from Machine Learning Models (Nandi et al., JCTC, 2021)"*. The stored PDF is arXiv:2103.05491 and `Papers/README.md` always named it correctly as Käser et al. **The title and authors here were wrong**; the significance was right. A bibliography entry written from recall is not a citation.
   - *Significance:* Demonstrated that neural-network potentials can carry CCSD(T)-level second and third derivatives, via transfer learning, well enough for VPT2 — within 20 cm⁻¹ of experiment for ~90 % of modes at the best level, while MP2-quality surfaces produce outliers up to 150 cm⁻¹. The level of theory under the model, not the model, sets the ceiling.
...
12. **MLMD Meets PAHs** (Mai, Wang, Pan, Schörghuber, Kovács, Carrete & Madsen, *MNRAS* 541, 3073, 2025)
    ...
    - *Significance (revised 2026-08-23):* the earlier summary — "brought the 2017 Gastegger MLMD methodology to complex aromatic systems" — badly understated this paper. It computes anharmonic IR spectra by machine-learning MD for **1,704 PAHs from the NASA Ames PAHdb, up to 216 carbon atoms**, at several temperatures, scaling linearly with system size.
    - **Consequence for this plan, recorded here rather than elsewhere:** classical MD + dipole-ACF FFT band envelopes — this plan's §9 deliverable — have already been produced at a scale far beyond H₂O and benzene. Anyone resuming this plan must read this entry first and decide what the contribution then is. This is the finding that caused the project to be re-planned as [plan 02](../../02_coupled-cluster-anharmonic-ir/).
...
15. **Observational Application to Current JWST Data** (*A&A*, 2026)
    - *URLs:* 
      ...
      - [JWST A&A Study 2](https://www.aanda.org/articles/aa/abs/2026/06/aa59999-26/aa59999-26.html) = Wang (2026), [arXiv:2602.12531](https://arxiv.org/abs/2602.12531)
    - *Significance (corrected 2026-08-23):* the earlier summary — "researchers directly identify the isotopic makeup of deep space PAHs" — **was wrong for Study 2** and is retracted. Study 2 is *Full-spectrum infrared fingerprinting*: a random-forest classifier trained on 23,000+ spectra assigning PAH **size and charge categories** (F1 = 0.963), with size diagnostics found to be charge-dependent. That is classification into categories, not identification of named species. Study 1 has not been re-verified; treat it as unchecked until it is.
