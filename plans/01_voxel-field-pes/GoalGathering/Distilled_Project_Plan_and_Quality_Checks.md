...
- **No spectral training:** the production network is trained only on static configurations (energies, derivatives, Hessians, densities and dipoles), never on spectra, peak positions or intensities. Once frozen, it runs forward-only classical MD for tens of picoseconds and the IR envelope is obtained by FFT of the dipole autocorrelation. Band positions emerge from the static PES; relative intensities come from the statically supervised dipole surface and frozen-weight dynamics. They are not wholly unsupervised.
...
- **Rigorously phased**, with hard numerical Go/No-Go gates between phases (§7), for a self-paced master's program beginning 2026-09-01 with 10 hours/week of human attention. Architecture development remains local-first. Copilot and compute may run outside those hours, but outputs require human validation. Limited supercomputer access is explicitly authorized for the frozen §5.1 higher-level label audit; production campaigns require their own measured pilot and may not silently consume that allocation.
...
- **NOT treating HPC as unmeasured production capacity.** The counted CCSD(T)/CBS(T,Q) reference audit in §5.1 is an approved, narrow use of limited supercomputer access because canonical benzene QZ work is not a credible consumer-PC promise. Architecture development stays local. Any production-campaign HPC use requires a separate measured pilot, allocation confirmation and written schedule update; otherwise the shrink ladder fires.
