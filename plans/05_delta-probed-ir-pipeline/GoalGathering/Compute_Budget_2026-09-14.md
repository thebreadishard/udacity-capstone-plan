...
| the same energy at the anchor thresholds (xtight) — the factor **F** over tight | **NOT_RUN**: the timing is in its last fragment as this is written (fragments 1–23 of 24 printed; the four largest fragments ≈ 3–4 h each); the value is the sum of the fragment times from the checkpoint file, not the calendar span | `results_timing/naphthalene_ccpvtz_xtight.log`, `naphthalene_cc-pvtz_xtight_fragments.json` |
...
The anchor runs at the tighter LNO thresholds [10⁻⁷, 10⁻⁸] ("xtight"): the benzene rescan brought the composite frequency bias from +0.47 / +0.03 / +0.79 to +0.11 / −0.01 / +0.23 cm⁻¹ on the three test modes with the smoothness unchanged (decision 20). The price is F per energy; the benzene factor is about 2, and F at naphthalene is the one timed energy this document waits for. Every naphthalene price below is written at tight and multiplied by F once printed; no R1 deck is priced for a purchase or a cluster application before that.
...
| R1 naphthalene | 11.5 h per tight energy, 19.8 GB; the 474-energy deck 5,450 h ≈ 227 days at tight, × F at the anchor | ≈ 200–300 SBU and 1.5–2.5 h wall per energy on one thin node → ≈ 100,000–150,000 SBU for the deck (× F); 4–5 days on one node, about a day on four; inside one Small Compute application |
