...
| Off-diagonal count | Sanders et al.: ~30 % of columns on anthracene in a cheap-method eigenbasis, ~log growth to 15 rings, DFT level (bib 24, fetched); CMA-2: at cutoff ξ = 0.02 the selected off-diagonals cost +33 % over diagonal-only for a mean maximum error of 0.17 cm⁻¹; per-molecule counts vary (bib 43, read; "~33 selected off-diagonals" was a misreading, corrected 2026-09-06) | **K_off** per rung — NOT_RUN; the quantity Q8(c) tests |
| Gradients for a full Hessian, DFT level | O1NumHess: a gradient count that levels off around 100–124 for hundreds of atoms; error about twice a conventional finite-difference Hessian's (bib 23, arXiv HTML read 2026-09-03; the earlier phrase "worst covalent case a conjugated polyene, MAD 6–12 cm⁻¹" is not in the bibliography's record of that reading and is withdrawn 2026-09-06 until the PDF is read) | K(G) per rung — NOT_RUN; mode G is the aimed-for route, built in the side project; K_cap(G), n_min(G) and c(G) are frozen from the noise-injected gradient-mode dry run for every rung |
...
**Dated note 2026-09-14 07:49 — the naphthalene xtight timing finished (measurement; changes no rule).** After three interruptions and two resumes, all 24 fragments are in the checkpoint: fragment solves summed over the segments **138,305 s = 38.4 h** (per fragment 2,094–14,318 s; the four largest, one symmetry class at 343 of 378 virtuals, 3–4 h each), the last segment alone 78,103 s; peak resident 15.35 GB with `--max-memory 16000` (out-of-core four-virtual blocks); E_corr(CCSD(T)) −1.6870085 E_h against −1.6866006 at tight. **F = 138,305 / 41,375 = 3.34** over the tight energy of 11 September — with the caveat that the tight run was in-core in one segment, so the ratio carries the out-of-core path as well as the thresholds (the benzene factor at equal settings was ≈ 2). The decision-33 basis terms cost 60 s (DF-MP2/cc-pVQZ, 790 bf) and 353 s (DF-SCF/cc-pV5Z, 1,350 bf) per point. Consequences: the R1 deck at the anchor thresholds is 474 × 38 h ≈ 18,200 laptop-hours ≈ 760 days; `DURATION_TABLE.md` reprinted (desktop 198–330 days, four Snellius nodes 25–41 days, both estimates); the 25 GB WSL ceiling was harmless for the run itself (three host-side resource-exhaustion events, all survived once nothing heavy ran on the Windows side), so it stays until a job needs otherwise. Scratch (20 GB) cleared after the run. Records: `results_timing/naphthalene_cc-pvtz_xtight.json`, `_fragments.json`, `naphthalene_ccpvtz_xtight.log`.
