...
## 7. Proposed amendment of 14 September, 11:0x — written AFTER the first column beyond A existed (column B, tetracene); not applied; the user decides

Column B (PAHdb Anharmonic v1.00, tetracene, 3,651 transitions) scored 22.8 cm⁻¹ against A's 22.9 on the intensity-weighted EMD, although its family weights sit on the argon-matrix shares (combination region 14.1 % against matrix 10.7, A 0.0) and its strongest-band position terms improve in five of six families. The within-family EMD rose (C–H out-of-plane 6.7 → 20.3 cm⁻¹) because term (1) compares a unit-normalised histogram of the table's 2–10 identified peaks per family with a dense stick list of 122–550 transitions per family: mass in any bin the table does not list is charged in full, so a physically richer spectrum loses to a sparse one. The flaw is in the metric's treatment of a *peak table* as if it were a spectrum. **Proposed change to term (1), to be adopted or refused by the user before column 0, P or N is scored:** convolve both the laboratory peaks and the predicted sticks with the FELIX bandwidth (a Gaussian of FWHM 1 % of ν) before binning, or, equivalently in spirit, keep only predicted sticks above the intensity of the table's weakest listed band in that family, and compute the EMD on the result; both the pre-registered and the amended number are printed for every column so that the change is visible. Terms (2) and (3), and the losing and winning conditions of §4, are unchanged. Until the user decides, column B stands as printed and no verdict is drawn from term (1) for any column.

**Applied 14 September, 11:2x, at the user's instruction ("Pas het amendement toe, beide getallen printen"):** the convolution form — a Gaussian of FWHM 1 % of ν applied to the laboratory peaks and the predicted sticks alike, mass integrated per 10 cm⁻¹ bin, truncated to the family window, unit-normalised — is implemented in `shape_score.py` as a second EMD column ("EMD conv.") and a second weighted number; the pre-registered raw-histogram EMD is still computed and printed first in every row. The threshold form is not implemented. The losing and winning conditions of §4 are read on the amended number from now on, with the pre-registered number printed beside it so the change stays visible; column A's baselines under both forms are in `out/shape_score.md`. One rule the convolution forced, declared here: in the amended term a family the laboratory populates but the prediction leaves without broadened mass scores the **family width** (the worst case) and stays in the weighted mean, whereas the pre-registered term excluded such a family (which is why column A's pre-registered number never paid for having no combination bands). Numbers after applying it (pre-registered / amended, cm⁻¹): tetracene A 22.9 / 45.1, **B 22.8 / 19.1**; coronene A 14.5 / 30.0; peropyrene A 18.2 / 14.9 (its list has no band above 1650, so no penalty); ovalene A 16.6 / 28.2; HBC A 19.2 / 29.9. The tetracene combination-region entry for A (116 cm⁻¹) is Gaussian tail mass at the region's low edge against laboratory bands spread over it, i.e. the worst case reached in practice rather than by the declared rule; both readings say the same thing.
