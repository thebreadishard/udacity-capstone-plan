...
***c* measured 15 September 15:26 (`probes/results_m4/m4_timing_benzene_cc-pvdz_tight.json`, quiet machine): benzene neutral LNOCCSD_T tight 164 s, benzene⁺ at the same geometry `ULNOCCSD_T` tight 5,096 s (15 + 14 spin-fragments, 4.2 GB, UHF ⟨S²⟩ 0.870 after one stability round) → c = 31.0.** Read into the rows: R1⁺ H deck at xtight ≈ 1,460 laptop-days, at tight ≈ 430, the diagonal H deck at tight ≈ 170 — **no cation deck fits the laptop with the installed unrestricted code**, and on the desktop only the diagonal deck at tight (45–75 days) would. So the remedy is software, as the paragraph below anticipated, and the plan should say so: the first item was a *split* of the 5,096 s between the unrestricted CCSD and the NumPy (T) kernel — **measured 17:02: UCCSD alone 1,264 s, so the (T) kernel is 3,832 s (75 %)** and c_UCCSD = 7.7 — because only the (T) part is a bounded port — (T) *is* most of it: an unrestricted compiled (T) kernel brings c to ≈ 8–9 (the UCCSD's 7.7 plus a compiled (T) of the restricted kind's cost), the R1⁺ diagonal H deck to ≈ 45 laptop-days at tight and the full H deck to ≈ 115 (desktop 30–50) — the diagonal deck becomes laptop-feasible, the full one a desktop item. **Proposed decision for the user: the unrestricted (T) port** (own software, ledger row when decided; test = benzene⁺ energy equal to the NumPy kernel's to 1e-8 E_h, time ≤ 1.5 × the restricted (T)).
