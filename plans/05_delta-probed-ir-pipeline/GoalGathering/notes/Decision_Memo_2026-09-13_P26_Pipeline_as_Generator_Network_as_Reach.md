...
**Addendum 2026-09-13 (afternoon), the user's words on the success criterion of the reach product.** (1) "Die 5 cm⁻¹ is een richtlijn": the band figure is a guideline, not the definition of success. (2) The goal is a network that is *better than what exists*, which means above all that it predicts the **spectral shape of larger PAHs** — the reach rungs R4–R6, the user confirmed on 13 September (evening) — more reliably than the existing methods (PAHdb's scaled harmonic spectra, the anharmonic library). (3) "Better than what exists" may also mean: **with an error of which we have grounded reason to believe that it is smaller than that of the methods that existed** — i.e. a stated, defended error budget can be the claim where no laboratory truth decides, provided the grounds are measured (hold-out molecules with measured corrections, T-1/T-2 transfer errors, the pipeline's own budget per family) and not asserted — this reading of "grounded" confirmed by the user the same evening. Consequences, none applied yet: the reach rungs' claim type becomes "error budget, grounded" rather than "beat at 5 cm⁻¹"; intensities and widths (lead C) belong to the shape and therefore to the reach product; the laboratory-decidability remedy (lead G) stays the cheapest way to *ground* the budget where it can be grounded. **Plan 07 (leads A + D, with C, the shape objective as its goal) is decided after F and g are known** (the user, same afternoon); no folder before that.
