...
**Outcome 10:0x — FAIL of the lever (dropped).** `out/E11_orbit_avg_2026-09-25.md`, 139 molecules averaged (74,537 pairs; median target change 0.079 of RMS, max 0.43 on a rotor). (a) Read-outs against the original dH_true, three seeds at 175: ratio (a) 0.43 [0.423–0.435] vs reference 0.43 [0.428–0.434], (b) 0.47 vs 0.47; corrected ω (a) 4.82 ± 0.05 vs 4.71 ± 0.06 (0.1 cm⁻¹ worse, two seed-sd), (b) 5.22 ± 0.07 vs 5.14 ± 0.23 — not better anywhere. (b) Per-pair RMS error on the 22 rigid hold-out molecules against the averaged targets: 0.00042 vs 0.00043 (−1.6 %; registered line ≥ 8 %). (c) Slope unchanged (0.46 → 0.43 vs 0.47 → 0.43). Why so little: the target's within-orbit noise is 0.103 of the signal and adds in quadrature to a model error of 0.31 of the signal, so even its complete removal from the reference is worth 5 %; and a model trained on 175 molecules already averages that noise across examples, so cleaning the labels changes the fit by nothing measurable. The orbit average stays useful as a *reference* (it is the better estimate of the truth on rigid molecules, and the noise principle's second route to E11.4) but not as a training lever. The layer-B pre-registration is unchanged.

*Caveat 12:4x (frozen recipe):* every curve and slope in this note (E11.5's 1.14× / 1.15× / 1.22× per decade, the E7 rung-B points they are fitted on) was measured with one frozen training recipe — no search over optimiser, loss function or the main hyperparameters. By the rule adopted today (QUALITY_POLICY; proof pre-registration, amendments of 12:4x) they are readings of one recipe, not statements about what the model can learn; the tuned curve (stages 1 and 2) is what those statements will rest on.
