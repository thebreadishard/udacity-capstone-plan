...
## Outcomes (running record)

- **26 Sep, 12:1x — E1 (band pool), first run, P1 and oracle, 97 evaluation molecules (41 parents never in training, 56 A2/B), three P1 seeds, read by `readout.py` under the fallback registered at 12:0x (P0 reaches ρ_off ≤ 0.3 on 1 of 41 and 3 of 56 molecules, so n_half and AUC are the lines):** n_half ratio vs P0 (median; fraction improved) on ρ_off — parents P1 0.17/0.20/0.19 (95 % improved), oracle 0.06; A2/B P1 0.20/0.17/0.17 (98 %), oracle 0.07. AUC ratios 0.82–0.87 (P1), 0.84–0.90 (oracle). In-band Frobenius n_half: P1 0.19–0.26, oracle 0.06–0.09. **S1 passes on both evaluation sets** (pass line ≤ 0.80 with ≥ 70 % improved): a scorer that has seen other molecules gets a new molecule halfway to the deck's final residual with about one fifth of the energies the hashed order needs, and the oracle shows another factor three of headroom. S4 (size transfer): the parents' ratio equals the A2/B ratio within 0.03 — the gain does not vanish with size. The prediction (P1 ≈ 0.5 / 0.7) was too pessimistic by a factor 2.5–3.5. Not licensed: anything about P2 (running), anything at CC level, anything about the wider pool (E2 running). Files: `out/sim/band_readout.{md,json}`, curves in `out/sim/band_merged.json` (local; 8 CPU-hours on hel1-23).
- **26 Sep, 14:3x — E1 (band pool), round 2 with all orderings (P1, P2 learned embedding at stage 0, P12 combination, oracle), same 97 molecules:** n_half ratio vs P0 on ρ_off — parents: P1 0.17/0.20/0.19, P2 0.20/0.21/0.21, P12 0.17/0.20/0.16, oracle 0.06; A2/B: P1 0.20/0.17/0.17, P2 0.20/0.25/0.20, P12 0.17/0.17/0.17, oracle 0.07. In-band Frobenius n_half — parents P1 0.19/0.21/0.23, P2 0.24/0.29/0.26, P12 0.20/0.22/0.18; A2/B P1 0.24/0.24/0.26, P2 0.30/0.33/0.27, P12 0.24/0.26/0.24. AUC ratios: P12 0.82–0.85 (best of all), P1 0.82–0.87, P2 0.84–0.88. Reading under the registered rules: **S1 confirmed** (P1 as before); **S2 not met at stage 0** on the band pool — P2 alone orders about as well as P1, slightly behind on the in-band Frobenius line; the combination P12 is at least as good as P1 everywhere and best on AUC, as predicted. No sentence about the embedding's value is licensed yet (the user, 12:1x): stage 1 of its search (lr × width, patience 20, 18 fits) is running on the laptop, 8 of 18 done; the all-pairs pool E2 — where the embedding is predicted to matter — is running on hel1-23 (1 of 97 at 14:3x, ≈ 3 h). Files `out/sim/band_p2_readout.{md,json}`; curves local.
