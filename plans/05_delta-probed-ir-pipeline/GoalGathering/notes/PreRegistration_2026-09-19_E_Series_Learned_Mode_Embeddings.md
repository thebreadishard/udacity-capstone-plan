...
### E5b outcome — 19 September, 23:0x: **lose** (`out/embedding_skipgram_E5b_2026-09-19.md`; balanced loss, 1,500 steps, three seeds)

| encoder | diag C–H stretch | diag C–H oop | diag ring | diag other | ring couplings RMS / zero | ratio | ring block RMS / median rule | probe ring |
|---|---|---|---|---|---|---|---|---|
| E5b-tok | 5.42 | 5.61 | 13.99 | 18.08 | 9.79 / 5.49 | 1.78 | 4.77 / 4.03 | 15.93 |
| E5b-atoms | 9.72 | 8.59 | 17.61 | 14.03 | 7.49 / 5.49 | 1.36 | 5.63 / 4.03 | 18.56 |

With the balanced loss the models train (final losses 0.13–0.15 and 0.07–0.08 in (K/50)² units) and the read-outs move off the zero rule, but every pre-stated reading is a loss: ring diagonal 14.0 (≥ 11.5; the Transformer's 12.4 is not reached), ring couplings *worse than predicting zero* (ratio 1.78 tokens, 1.36 atoms — the bilinear form invents couplings that do not transfer), ring block 4.8 against the median rule's 4.0. The atom-set encoder is worse than the token encoder on every diagonal, as in E1b.

**Reading of the whole E-series, one evening, 45 molecules.** (1) E4 is the result that stands: for the ring family the per-mode label is ill-posed (9.2 cm⁻¹ of the 12.4 is definitional), the family block's mean transfers to ≈ 4 cm⁻¹ by the median rule alone, and the within-molecule distribution of the correction — the couplings — is what does not transfer. (2) E5b adds the sharper statement: with 30 training molecules the couplings are not predictable across molecules by a rank-32 bilinear embedding from either tokens or raw atoms; they behave as molecule-specific mixing at this data size. The word2vec analogue was implemented faithfully the second time and lost; whether it wins at 1,000 molecules (word2vec's own regime is millions of sentences) is a question for the corpus, not for tonight. (3) Nothing tonight beats the mode-token Transformer on the diagonal, and nothing beats the median rule on the block. (4) The next experiments in this line are, in order of cost: the BHHLYP − B3LYP proxy on benzene and naphthalene (is the ring correction of *this* proxy non-local?); layer A2 of the corpus (868 molecules, ≈ €100–150 of rented machine time) to give any raw-input model the data it needs, with E5b and the design note's Stage 1 re-run on it as the pre-registered scaling test; and the block-level target in the recipe, adopted tonight, which is independent of both.
