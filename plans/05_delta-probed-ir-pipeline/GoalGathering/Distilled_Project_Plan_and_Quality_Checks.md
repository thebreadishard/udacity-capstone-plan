...
| **Hold-out and residual ρ; the noise floor** | a fraction f_h of patterns, chosen by the seeded deck rule before any response exists, never enters the recovery. **ρ(n)** = RMS over held-out patterns of (response predicted by the recovered Δ₂ − computed response) ÷ RMS of the computed held-out responses; dimensionless; per mode. **ρ_noise** = σ(mode)/RMS_resp(rung), σ the per-point scatter from the R1 smoothness probe (Q6 estimator), RMS_resp the rung's own held-out response RMS. ρ is the error-budget term and the P3 metric |
| **K** | measured with the **noise-aware stopping rule**: the smallest pattern count n, in hashed order, at which ρ(n) ≤ ρ\* = c·ρ_noise (c ≥ 1, pilot-note item 8) — equivalently the held-out χ² per point with σ as the per-point sigma first falling to c²; K_off = K − 2M in mode E. Reported in the cost record. Capped by K_cap (pilot note item 9, from the noise-injected dry run, per mode). A residual that never reaches c·ρ_noise by K_cap is "not recovered at cap" |
...
- **The controlled comparison (frozen): learned prior vs structural prior at matched K, on the
  ...
  well.** Same patterns, same held-out set, same solver, ≥3 seeds for the prior: does the prior
