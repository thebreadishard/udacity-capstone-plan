...
| Anchor level | local CC = **DLPNO-CCSD(T) or LNO-CCSD(T)**, code and thresholds fixed in the deck — **LNO-CCSD(T) in PySCF/PySCFAD is the preferred candidate** because it is the one code in which domains can be frozen and gradients obtained (side project 2026-09-04); deck field `PNO extrapolation: none | CPS(6/7)` (bib 44); **domains, pair lists and per-pair PNO counts frozen at the reference geometry for every displaced evaluation**; **licensed by measured deltas against frozen formulas, not by trust** (Q6) |
...
| **Modes** | **E** (energies; the guaranteed route) or **G** (gradients; the aimed-for route — on the 2026-09-03 landscape no production code offers a local-CC(T) nuclear gradient, bib 31–34; PySCFAD's AD gradients are demonstrated to 29 atoms, bib 33; plan 05 builds the frozen-domain version in the pre-registered side project of 2026-09-04, whose milestones M3/M4 license mode G on R1/R2). Chosen per rung by the gradient-availability probe, which prints wall-clock and peak memory; both printed where both exist |
