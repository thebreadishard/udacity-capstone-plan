...
| scaled, ML-corrected or anharmonic DFT for PAH spectra | PAHdb v4.00 (Ricca et al. 2026); the PAHdb Anharmonic library v1.00 (Mackie et al. 2015, 2016; Esposito et al. 2024); Mulas et al. 2018; the ML-corrected scale factors of Bos et al. 2025 (marketed as Ethereal AI) | these are the opponents; the plan adds a measured coupled-cluster correction to the harmonic constants and an error budget per band, and leaves the anharmonic constants at DFT level as they do. **Added 13 September (software record, paper not yet found in Crossref, OpenAlex or arXiv):** the group of line C has published code for a machine-learning double-harmonic pipeline — a neural-network force field for harmonic frequencies and modes and an electron-passing network for dipole derivatives, charge-state-specific, pre-trained — for high-throughput PAH infrared spectra (`zwAstroChem/ML-DH-PAH-IR`, created July 2026; components Montes-Campos et al. 2021 and Metcalf et al. 2020, both Crossref-verified). It is the nearest existing "large PAH in, harmonic spectrum out" product and enters the opponents atlas as soon as its paper and training level can be read; this plan differs in the label (a measured coupled-cluster correction) and in the error budget per band |
