...
- **Accuracy rungs (A).** Laboratory data exists. The claim is *beat the frozen line per band
  ...
  **Dated amendment 2026-09-08 (P20, decision 28 — the user: benzene is a tool, not a goal).**
  *R0 is an agreement rung*: the claim is **agree with each scored laboratory band within the
  laboratory band uncertainty combined with the pipeline's own error budget** (noise, freezing
  bias, basis-set line, locality) — for positions and, under decision 18, for the integrated
  intensities against the source's stated uncertainty. Passing licenses the anchor and the
  recovery; failing means the budget is incomplete and nothing moves up the ladder. The paired
  comparison with the opponents is **printed at R0 but claims nothing**; the word "beat" is not
  written for benzene. *R1 does both*: agreement is required (the local-CC licence rung) and, per
  family, the question whether the coupled-cluster correction adds accuracy over DFT is answered
  on the PNNL column and Pirali's resolved fundamentals (decision 21). *R2–R3*: "beat" per family
  where the laboratory can decide it, as written below. The Goal's value hierarchy ("beating the
  lines on benzene is not the point") already said this; the Ladder now says it too.
...
source states a narrower width; (ii) **coronene**, jet-cooled at T_rot ≈ 2 K, six tabulated
6–15 µm bands (item 62; written as five until 2026-09-06 — the 770.1 cm⁻¹ band had been missed), same bandwidth caveat — the first gas-phase datum for R3;
...
- **[05] Frozen spaces — the object, written once.** At the reference geometry the local-CC code
  ...
  **B**, never C, so localiser arbitrariness is not attributed to freezing; M1 prints all three. Every local-CC probe
  ...
  coefficients (they would otherwise make three benzene Δ₂,ii readable before the note). **"Sealed"
  means (defined 2026-09-06 after the literature cold read): the file is committed in plain text
  and readable in the public repository; its SHA-256 sidecar proves it was not altered; and the
  author has undertaken not to open it before the pilot note. It is a tamper seal and a discipline,
  not secrecy — the plan's "measured, not asserted" rule wants the raw evidence in the tree.** A code that cannot
...
2. The "beat" margin per family, from the lab and opponent side only; the list of promised
   families closed in the same note; and the **expected-effect line**: "the only literature figure for the
   harmonic CC−DFT difference on this ladder is **benzene's**: MAD 5.45 cm⁻¹, B3LYP/N07D vs
   CCSD(T)-F12b/cc-pVTZ-F12 (item 45, read in full 2026-09-06); **no literature figure exists at
   R1** — the R1 expected effect is a stated expectation, not a citation [this line rewritten
   2026-09-06 after the literature cold read; it read "≈ 5 cm⁻¹ at R1, item 45, snippet grade"]; the
   P2 hypothesis is that the per-family scatter of Δ₂ exceeds the margin
