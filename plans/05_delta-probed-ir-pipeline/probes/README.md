...
1. **Zero-CC dry run, both modes** (`dryrun_dft_delta_recovery.py`): Δ between B3LYP and a
   ...
   grid (the fixed "declared ρ" was retired by decision 12, 2026-09-06; **K in energies, a ± pair counting 2, exactly as probe 6** — and, by decision 37 of 14 September, a second count printed beside it: one energy per irrep-pure non-totally-symmetric pattern, a pair per totally symmetric pattern and per noise witness; the naphthalene dry run is the confirmation that makes the second count the rule), the off-diagonal blocks flagged large, the *15 Sep:* confirmed at DFT level on the symmetrised naphthalene stage A (odd parts ≤ 0.001 µE_h; `results_dryrun/i14_odd_part_dft.md`).
...
2. **Probe M1 — frozen spaces** (`m1_frozen_spaces.py`; runs in WSL, `~/qc05` — pyscf-forge 1.1.1,
   ...
   stop 1.
2b. **Probe M1-μ — the frozen-space dipole** (`m1_frozen_dipole.py`, owed after the cc-pVTZ M1 scan;
   decision 18, Ladder §3 dated note): μ(q) of arms A and B along the three M1 benzene modes, nine
   points, relaxed CC density or finite field in the frozen spaces, against canonical CCSD(T);
   prints σ of μ(q) about a degree-4 fit per component and the bias of ∂μ/∂q at q = 0 in km/mol per
   mode; no verdict. Tests item 30's objection on the plan's own object. The intensity scoreboard
   for R0 and R1 (integrated band intensities from items 56, 57, 59) is generated by 2a alongside
   u_band, with the source's stated intensity uncertainty per band.
