...
## Dated note 2026-09-25 07:2x — section 9 (E9, E10, size extrapolation) appended; a repair of the executed notebook

**Repair first.** The append-only executor of 24 September ran definition-only copies of the two heavy cells (12: training, 17: evaluation) so that the new cells could use their functions; nbclient stores the executed cell object at the cell's index, so the copies replaced the real cells in the saved notebook — sources truncated to the definitions, the 23 September training and evaluation outputs lost, execution counts reset. Noticed today when the executor's own check ("code cell changed") fired on cell 2 for a different reason. Cells 12 and 17 were restored byte-for-byte from commit 69d9eab (the executed run of 23 September: 40 and 50 source lines, 2 and 3 outputs, execution counts 7 and 10); the notebook metadata records the repair (`repair_2026_09_25`). Section 8's cells did not depend on those outputs (8.2 reads results.json), so nothing downstream changes. `execute_section8.py` now keeps the original cell objects around every temporary execution and regenerates the scratch notebook with the executed run's release name (`M05_RELEASE` parsed from cell 2), which was the cell-2 mismatch.

**Section 9.** Cells 39–50 added by `make_notebook.py` and executed append-only (`execute_section9_2026-09-25.log`; setup cells 2, 4, 8, 10 and the definitions of 12 and 17 rerun; new code cells 41, 44, 47, 50; no errors): 9.1 E9 core-transfer table and figure (`figures/e9_core_transfer.png`) from `data/e9/e9_core_transfer_2026-09-24.json` and the post-hoc energy-only variant; 9.2 E10 per-substituent table from the registered and torsion-matched runs; 9.3 the size-extrapolation curve (`out/E7_rungB_size26_2026-09-25.json`, `figures/size_extrapolation.png`); 9.4 what was learned; `results_followup3.json`. Report rebuilt with Addendum 3 (`make_summary.py`, docx + PDF). All numbers trace to the three pre-registrations of 24–25 September and their outcome sections.

## Dated note 2026-09-25 10:5x — section 10 (controls: shuffled labels, noise floor by two routes, the withdrawn symmetry reading, orbit-averaged labels, error per class, predictions on record) appended

**Section 10.** Cells 51–71 added by `make_notebook.py` and executed append-only (`notebook/execute_section10_2026-09-25.log`; setup cells 2, 4, 8, 10 and the
definitions of 12 and 17 rerun in a fresh kernel, sections 1–9 kept with their saved outputs; metadata `append_only_execution` updated). No training: every
cell reads result files of the pre-registered desk tests E11.1–E11.8 (`out/E11_*`, `out/E7_rungB_2026-09-23_analytic.json`) and the two per-pair dumps
(`out/E11_orbit_dump_…_pairs.npz`, `out/E11_orbit_avg_…_pairs.npz`, 2.5 MB each, not committed, regenerated in 30 s by the commands in the E11
pre-registration). Outputs: `notebook/results_followup4.json`, `figures/power_law_predictions.png`. Section 10.3 records a reading that was wrong for about
an hour on 25 September (symmetry spread 0.52 read without its target control) and its correction; it is kept in the notebook on purpose. The noise-floor
