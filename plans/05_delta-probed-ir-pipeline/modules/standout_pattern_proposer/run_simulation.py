...
def evaluate(e: dict, orders: dict, stride: int, noise_sigma: float) -> dict:
    res = {}
    f = np.asarray(e["freq_cm"])
    inband_pair = np.array([abs(f[i] - f[j]) <= C.W_BAND_CM and i != j for (i, j) in e["pairs"]])
    for name, order in orders.items():
        curve = C.rho_curve(e, order, stride=stride, noise_sigma=noise_sigma, inband_mask=inband_pair)
...
def main() -> int:
    ...
    if a.limit:
        evals = evals[: a.limit]
    print(f"{len(evals)} evaluation molecules ({sum(r['split'] == 'eval_parents' for r in evals)} parents); pool {a.pool}; seeds {seeds}; stride {a.stride}; noise σ {a.noise_sigma}"
    ...
    for k, r in enumerate(evals):
        ...
        for s, sc in scorers.items():
            orders[f"P1_seed{s}"] = C.order_by_scores(e, S.scores_matrix(e, sc.predict(X), pairs))
        per_mol[r["id"]] = dict(split=r["split"], M=e["M"], **evaluate(e, orders, a.stride, a.noise_sigma))
    ...
    for split in ("eval_parents", "eval"):
        ...
        summary[split] = {key: {name: ratio_summary(sub, "P0", name, key) for name in per_mol[next(iter(per_mol))] if name.startswith(("P1", "P3"))}
    ...
    out = dict(date=time.strftime("%Y-%m-%d %H:%M"), pool=a.pool, seeds=seeds, stride=a.stride, noise_sigma=a.noise_sigma, exports=str(a.exports),
               scorer_prefix=str(a.scorer_prefix), summary=summary, per_molecule=per_mol, seconds=round(time.time() - t0, 1))
    json.dump(out, open(a.out_prefix + ".json", "w"), indent=1)
    lines = [f"# Pattern-proposer simulation — pool {a.pool}, {out['date']}", "", f"seeds {seeds}, stride {a.stride}, noise σ {a.noise_sigma}; {len(per_mol)} evaluation molecules.", ""]
