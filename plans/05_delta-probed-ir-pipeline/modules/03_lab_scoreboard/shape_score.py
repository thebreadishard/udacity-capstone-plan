...
CONSTANTS = {"bin_cm": 10.0, "window_cm": [550.0, 2000.0], "families_scored": ["CH-oop (10.5-15 um; benzene nu11 at 673 included)", "ring / CH-ip (9-10.5 um)", "CH-ip-bend (8.6 um)", "CC-stretch/CH-ip (7.7 um)", "CC-stretch (6.2 um)", "overtone / combination region"],
             "position_term_min_fraction": 0.25, "line_A": "PAHdb theoretical v4.00 as served (Module 02 bands.csv.gz, frequency_cm = served scaled frequency)",
...
def score(lab_f, lab_i, st_f, st_i):
    ...
    tot_lab = lab_i.sum(); tot_st = st_i.sum(); rows = []; wsum = 0.0; wemd = 0.0
    for fam in CONSTANTS["families_scored"]:
        ...
        rows.append({"family": fam, "n_lab": int(ml.sum()), "n_sticks": int(ms.sum()), "share_lab_pct": 100 * share_lab, "share_pred_pct": 100 * share_st,
                     "family_weight_error_pp": 100 * abs(share_lab - share_st), "emd_cm": e, "strongest_lab_band_cm": (float(lab_f[ml][int(np.argmax(lab_i[ml]))]) if ml.any() else None), "position_term_cm": pos})
        if e is not None:
            wsum += share_lab; wemd += share_lab * e
    return rows, (wemd / wsum if wsum else None)
...
def main():
    ...
    for mol, uid in CONSTANTS["uids"].items():
        ...
        res = {}
        L += [f"## {mol} — laboratory bands in window: {int(((lab_f >= 550) & (lab_f < 2000)).sum())}; line A uid {uid}, sticks in window: {int(((cols['A'][0] >= 550) & (cols['A'][0] < 2000)).sum())}", "", "| column | family | n_lab | n_sticks | lab share % | pred share % | weight error (pp) | EMD (cm⁻¹) | strongest lab band | position term (cm⁻¹) |", "|---|---|---|---|---|---|---|---|---|---|"]
        for name, (sf, si) in cols.items():
            rows, wm = score(lab_f, lab_i, sf, si); res[name] = {"rows": rows, "weighted_emd_cm": wm}
            for r in rows:
                L.append(f"| {name} | {r['family'].split(' (')[0]} | {r['n_lab']} | {r['n_sticks']} | {r['share_lab_pct']:.1f} | {r['share_pred_pct']:.1f} | {r['family_weight_error_pp']:.1f} | {('%.1f' % r['emd_cm']) if r['emd_cm'] is not None else '—'} | {r['strongest_lab_band_cm'] or '—'} | {('%.1f' % r['position_term_cm']) if r['position_term_cm'] is not None else '—'} |")
            L.append(f"| **{name}** | **intensity-weighted EMD over families** | | | | | | **{wm:.1f}** | | |" if wm is not None else f"| {name} | (no overlap) | | | | | | | | |")
