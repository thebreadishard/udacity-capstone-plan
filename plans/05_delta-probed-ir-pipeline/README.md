...
**Decision 36 (2026-09-14 — P26 adopted with one change: the network as the reach product, thin decks above naphthalene, full decks above naphthalene conditional on probe M3):** the dated revisions of `Draft_2026-09-14_P26_Revision_and_Cover_Paragraph.md` §2–§3 are applied (proposal §1, new §3.5, §6, §12, §13 item 5; Ladder §2 dated revision and §3 bullet; cover note paragraph with the two corrections named there), each marked *(dated revision 14 September, decision 36)*; the one change against the morning's texts: every sentence "no machine can pay a full deck above naphthalene" is qualified "at the anchor's basis", with probe M3 (`notes/PreRegistration_2026-09-14_M3_DZ_Anchored_Decks.md`) named as the measurement that could return full decks to the ladder, and the Snellius request sized both ways (≈ 330,000–500,000 SBU at cc-pVTZ; ≈ 30,000–45,000 if M3 licenses cc-pVDZ). The user's words (16:4x): "Akkoord met al je voorstellen."

**Decision 37 (2026-09-14 — I14 adopted as a dated amendment of the K rule, confirmation by the naphthalene DFT dry run):** one energy per irrep-pure non-totally-symmetric pattern, a pair per totally symmetric pattern and per noise witness (Ladder §3 bullet; probes README item 1 prints both counts); measured admissible at tight and xtight on benzene's b₂u mode (`probes/m1_odd_part_symmetry.py`); effective from the first deck after the dry run (queue item 5) reproduces the direct Δ₂ within τ₇ with irrep-pure single-sided patterns; the mandate ledger's I14 and the plan-06 cost ladder's lever H carry the numbers. **Hard prerequisite added 2026-09-15 02:5x (measured on M3's first mode):** the geometry symmetrised to ≤ 10⁻⁸ bohr and every pattern projected onto its irrep before any single-sided energy — naphthalene's factory geometry (D₂h to 4–7 × 10⁻⁵ bohr) gives an odd part of 23–34 µE_h per unit q along a pure non-totally-symmetric mode, comparable to the response; benzene's ≤ 0.55 µE_h was that small only because its stage-A geometry was symmetric to 10⁻⁶. The dry run of item 5 starts from a symmetrised geometry for the same reason. *Confirmed 15 September 16:4x by the naphthalene dry run:* factory (`symmetry c1`) modes carry odd parts up to +30.9 µE_h at q = 1 (BHHLYP, mode 12) through their 1e-4 Ag admixture; the symmetrised, irrep-projected modes of stage A `--symmetrised` give ≤ 0.001 µE_h for modes 12 / 22 / 31 in both functionals (`probes/results_dryrun/i14_odd_part_dft.md`); the prerequisite is met and is now part of stage A. The second K count (291, `probes/deck_counts_planar.py`) stands.

**Decision 38 (2026-09-14 — the anchor's thresholds and the machine order after M2a):** (a) the anchor stays at **xtight** (bias +0.11/−0.01/+0.23 cm⁻¹ at benzene; 38 h per naphthalene energy at cc-pVTZ, ≈ 3.3 h estimated at cc-pVDZ if M3 wins); M3 itself runs at tight in both bases as pre-registered. (b) The σ-run of decision 35 does not start tonight. (c) Order after M2a: **probe M3 first** — its DZ cells (≈ 13 laptop-hours) then its TZ cells (≈ 6 laptop-days) — then the σ-run (4.8 laptop-days); M3's own ± pairs along non-totally-symmetric modes give a first σ(R_s) estimate on the way (decision 37). The naphthalene DFT dry run (item 5, Windows psi4, hours), the pyVPT2 benzene run (≈ 3 h) and the parked app update take the gap between M3's DZ and TZ cells (15 September midday); no psi4 beside a WSL anchor job.

**Decision 39 (2026-09-14 evening — the ladder is built slowly, ring by ring, with ions; the user: "Laten we traag doen, met ionen"):** the size ladder gains **rung 3 (anthracene and phenanthrene, the isomer test at equal size)** between naphthalene and the pyrene class, with full decks in cc-pVDZ if M3 licenses the basis (≈ 2–3 desktop-weeks together, estimate) and thin decks otherwise; rung 4 (pyrene, tetracene) full if M3 wins, thin otherwise; rung 5 (perylene or pentacene) only after rung 4 and only if the per-family transfer 3 → 4 held; **cations enter at rungs 1–2 (benzene⁺, naphthalene⁺)** — the gap of mandate-ledger obstacle 9, closed where it is cheapest, unrestricted local CC via `ULNOCCSD_T` after a smoke test and one timed energy. **The go/no-go per band family is read after rung 3** (transfer 2 → 3 within 2.5 cm⁻¹ for both isomers; the price curve over three sizes), ≈ 2–3 desktop-weeks after naphthalene instead of at the pyrene thin decks of March 2027; the extra desktop-months of rungs 4–5 are finishing work after that decision, not its precondition. Texts: proposal P27 with the cost table once M3's DZ number exists (15 September), then dated revisions of the Ladder §2, the proposal §5.2/§12 and the P26 label plan; the reach product and the shape objective are unchanged.

*Dated note, 15 September 07:xx–08:xx:* P27 is drafted on the measured cc-pVDZ price — `GoalGathering/notes/Draft_2026-09-15_P27_Slow_Ladder_Priced.md` (proposal, open; six decisions for the user). Its decks are counted from the point group (`probes/deck_counts_planar.py`): the three-ring isomer pair is 2,250 energies as full decks (phenanthrene's C₂v allows 584 same-irrep pairs against anthracene's 277), so the "≈ 2–3 desktop-weeks" above is the price of **diagonal-first decks at tight** (334 energies), which is what P27 proposes for the go/no-go; naphthalene⁺ is proposed as the first cation deck and benzene⁺ as a timing point only (Jahn–Teller). Nothing of it is applied.

**Decision 40 (2026-09-14 evening — the proposal goes to the supervisor on 26 September 2026; the user: "eind september mag van mij nog. Laten we dit plan volgen 26 september"):** the promise of "mid September" becomes a fixed date, 26 September, and the run queue of decisions 38–39 is read against it. By then, measured: the M3 DZ cells (15 Sep), the M3 TZ cells (≈ 21 Sep, the decisive number for the label budget and the Snellius paragraph), the naphthalene dry run with the P25 and I14 licences (15 Sep), the benzene VPT2 calibration (15 Sep), one timed benzene⁺ energy (the first ion row), the corpus price (done). Running but not finished on the day: the σ-run of decision 35 (≈ 26 Sep) — the proposal cites it as running with its pre-registration. Texts by 26 September: P27 applied as dated revisions (slow ladder with ions, peri main axis, acene stress branch, anchor diagnostics per rung, go/no-go after rung 3), the affordability table with both bases, the cover note, plan 06's annex; the pilot note itself stays a later document (its inputs are R0's, not yet run). Rung 3 and everything on the desktop are written as the plan, not as results.

**Decision 41 (2026-09-15 evening — the unrestricted (T) port; the user: "Absoluut ja op de (T)-port! Dat is het type voorstel dat we nodig hebben"):** the cation price measured today (M4: c = 31.0 at benzene, cc-pVDZ tight; the NumPy (T) reference kernel of pyscf-forge's `ULNOCCSD_T` is 75 % of it) is answered with own software: an unrestricted (T) kernel of the compiled kind for the LNO fragment energies (PySCF's compiled restricted kernel as the model), so that c falls to ≈ 8–9 and the naphthalene⁺ diagonal H deck becomes ≈ 45 laptop-days at tight (P27 §5). Acceptance tests fixed now: the benzene⁺ energy equal to the NumPy kernel's to 1e-8 E_h on the same fragments; time ≤ 1.5 × the restricted compiled (T) for the neutral; the shipped CH₃ comparison against canonical UCCSD(T) unchanged. Desk work of one to two weeks, in the private part of the repository, Software Changes Ledger row 13; nothing is sent upstream without the user's word. It starts after the 26 September proposal work is safe, and never displaces the TZ cells.

**Decision 42 (2026-09-16 morning — the cold reads of 16 September and what is done with them; the user: "Akkoord op al je adviezen"):** two fresh readers read the plan-05 proposal, cover note and Ladder §1–3 and the plan-06 annex, decision rule, README and cost ladder as the supervisor would (`notes/ColdRead_2026-09-16_Proposal_and_Cover_Note.md`, plan 06 `GoalGathering/ColdRead_2026-09-16_Annex_and_Decision_Rule.md`). Decided: **(1)** a **consolidated reading copy** of the proposal for 26 September, the dated revisions worked into one voice with a change log at the end; the dated original of 6 September stays as the record. **(2)** The misattribution of Esposito et al. 2024 to the supervisor's group (§1, two places) is corrected at once by dated note. **(3)** §13 item 5 is split into three numbered asks with a one-line derivation of the SBU figure on the decision-37 deck. **(4)** The cations enter the proposal explicitly as rung R1⁺ with the (T) port (decision 41) as its condition (P27 §5/§6), instead of the orphan "and charge state". **(5)** The plan-06 annex gets an opening paragraph with its four outcomes in plain words and one question to the supervisor. **(6)** The remaining stumbles of both reports are worked into the reading copy and the annex without further questions. Dispositions are written under each report. *Status 16 September 10:5x:* the reading copy is drafted (`GoalGathering/Project_Proposal_2026-09-26_Reading_Copy.md`, 1,792 lines, number-audited; three markers for the student; change log at its end) and is under a second cold read; the annex, decision rule, cost ladder and plan-06 README carry their fixes (items 5–6 done).

**Decision 43 (2026-09-18, 00:0x — M2, the in-house gradient of the frozen-space energies, is built in Python on JAX/PySCFAD, not in Rust; the user: "Okay, goed. Goed plan."):** M2b (17 September) showed that PySCFAD's shipped LNO does not compute the plan's quantity (5.5 / 8.2 / 26.4 µE_h against a bar of 6 on benzene's three probe modes; half the C–C response at its defaults), so the gradient engine is written, not borrowed: the frozen-space construction in JAX-traceable operations, PySCFAD's fragment CCSD(T) under automatic differentiation, `stop_gradient` on the reference vectors with the transport on the tape; the (T) port of decision 41 stays in C. Its done-criterion, float64 and checkpointing requirements, predicted cost ratio (2–4, falsified above 6.04) and the gate into a plan-05 deck are pre-registered in `GoalGathering/notes/PreRegistration_2026-09-18_M2_Done_Criterion_and_Gate.md`; plan 06's decision rule 2 transfers M2 through that document. The build starts after the anchor run's REPORT.md and on the user's word.

**Decision 44 (2026-09-18 morning — the conversation with the supervisor moves from 26 to 28 September 2026; the user: "Ik verplaats het gesprek naar 28 september"):** stage 0 of probe B1's cc-pVTZ cells passed on 18 September at 04:28, but one TZ tight energy costs 12–17 h, so the third mode and the report land ≈ 24 September; with 28 September all three modes are measured before the conversation. The reading copy keeps its filename and carries the new date in its header, in decision 40's text and in the §12 calendar row.

**Decision 45 (2026-09-20 evening — the naphthalene noise run of decision 35 is replaced by option B; the user: "Doe B, na het rapport van vrijdag"):** σ for the naphthalene labels' error budget is read from the anchor's own fifteen tight cc-pVTZ points (five per mode on modes 12, 22, 31; q = −1, −0.5, 0, 0.5, 1) plus a densification of **four points on mode 12** (q = ±0.25, ±0.75; the family with the largest quartic content, c₄/k 12 % at DZ, 6 % at TZ), instead of the 9-point single-mode run of decision 35 (≈ 10 energies, 4.8 laptop-days). Cost ≈ 4 × 12.2 h ≈ 2 laptop-days; the in-plane families get their σ from the anchor's points at the coarser spacing, and the error budget says so. Start: after REPORT.md (≈ 25 September, 00:00–02:00), on the laptop, as `m1_frozen_spaces.py … --modes 12 --npts 9 --resume` in the same output directory (the resume path reloads the sealed spaces and keeps the five existing points; to be confirmed on the dry-run that only the four new q values are computed). Nothing else starts on the laptop until this is done. Pilot-note input (f) is then met for three families with the spacing stated per family (`notes/Desk_2026-09-20_Pilot_Note_Inputs_Inventory.md`).

*Outcome, anchor and start of decision 45 (24 September 2026, 21:2x):* the anchor finished 24 September 21:02 (mode 31 read: between — `probes/results_m1/M3_TZ_MODE31_READING_2026-09-24.md`; probe M3 closes lose / win / between). The densification started 21:24 with `--npts 9 --modes 12 --resume` in the same directory; the live log confirmed 15 points kept and only the four new q values computed; expected end ≈ 26 September midday.

**Decision 46 (2026-09-21 — anharmonic step on analytic Hessians; the third step-size point; the presentation of the noise demonstration; the user: "1: keuze A. 2: draaien derde punt. 3: akkoord"):** (A) the anharmonic step of the spectrum pipeline (sheet 8) runs pyVPT2 on analytic pyscf Hessians wherever psi4 has no analytic Hessian for the functional; the sheet-8 caption reads "VPT2 (pyVPT2 on pyscf Hessians)". Grounds: the pre-registered tests T1/T2 of 21 September (`probes/results_vpt2/PREREGISTRATION_2026-09-21_FD_noise_demonstration.md`): with psi4's finite-difference Hessians at step 0.05 the two routes to φ_iijj disagreed by a median of 22.4 cm⁻¹ (max 1,265) and the fundamentals were unusable; with analytic Hessians at the same geometries and step the disagreement is 0.1 (T2), with a step of 0.20 and FD Hessians 2.3 (T1); both as predicted. (B) T3 runs: analytic Hessians at step 0.10 (61 pyscf Hessians, ≈ 3–6 h on hel1-14, no laptop work) to bracket the step-size systematic between T1 (breathing −28) and T2 (−16); the pipeline step follows from it. (C) For 28 September the demonstration enters as one paragraph in the cover note with the pre-registration note as an annex; it is not announced as a paper (gate E stays a candidate until the same diagnostic has run on Mackie's molecules). Standing rule from the same day: a default-valued keyword never goes into a task's keywords, because the checkpoint key hashes them (an explicit `findif__points: 3` cost an 88-minute recomputation).

*Outcome of decision 46, third point (21:2x):* T3 (analytic Hessians at step 0.10) gives the same fundamentals as T2 at 0.05 to 0.1 cm⁻¹ (bands 850.6 / 1004.4 / 1324.0; breathing −17.4) with the route maximum halved (0.44 cm⁻¹). The step-size question is closed: the pipeline runs the anharmonic step at 0.10 on analytic pyscf Hessians, and T1's −28 was the psi4 finite-difference input, not the quartic step. Sheet 8 stands as "VPT2 (pyVPT2 on pyscf Hessians)".

*Dated note 2026-09-22 14:2x — R0 diagonal deck, the geometry term (proposal for decision 48):* the composite curvature along the B3LYP modes at the B3LYP geometry differs from the literature CCSD(T) harmonics by −45 cm⁻¹ for C–H stretches and ±30 for out-of-plane modes while the in-plane modes agree to 2–8; xtight LNO thresholds change nothing; an HF test reproduces the effect (+111 cm⁻¹ from moving to HF's own a1g minimum, +114 predicted by our B3LYP cubic constants). The corrected harmonic of the pipeline therefore needs one high-level gradient per molecule (the first-order geometry term through the low-level cubics) beside the curvature; the corpus labels carry the same term. Details: `probes/results_m1/R0_DIAGONAL_READING_2026-09-22.md` (follow-up section). Decision open for the user.

**Decision 47 (2026-09-21 evening — code quality policy; the user asked for a policy and its enforcement before publication, and answered the proposal with "Akkoord"):** two tiers with a mechanical gate. Tier 1 (probes, module scripts, scratch) stays fast: compile-check, one dry run, provenance on every result file. Tier 2 (`src/dpir/`, the code that ships or is cited) enters only through a promotion checklist: pure functions, known-answer tests, invariance tests (the noise principle as assertions), a negative control for the defect that motivated the promotion, bit-for-bit reproduction of the probe's committed arrays, provenance block, review by a second reader, green CI. Enforcement: the gate is mechanical; every ledger incident becomes a test, a tool or a dated reason within a week; a publication freeze with a clean-machine reproduction, external code review and a Zenodo release. Written down in `QUALITY_POLICY.md`; first promotion the same evening (`probes/qff_from_hessians.py` → `src/dpir/qff.py`, 19 tests, CI `.github/workflows/plan05-ci.yml`, pre-commit hooks).

**Decision 48 (2026-09-22 17:1x — the geometry term is a fixed step; the user: "1 en 2: akkoord"):** a corrected harmonic frequency is the curvature measured at the low-level geometry *plus* the first-order geometry term — the shift towards the high-level minimum, Δω_i = ½ Σ_k φ_iik ΔQ_k, with ΔQ_k the relaxation along the totally symmetric modes read from the high-level gradient (or, as in the R0 deck, from the odd part of the ± energies along those modes) and φ the low-level cubic constants of the two-route QFF. Found on the R0 deck (C–H stretches 45 cm⁻¹ under CCSD(T) until the term was added: MAE 52 → 14), validated on HF (six modes, ≈ 10 %). Sheet 4: the label carries a geometry term per mode (new step `Geometry term (own software)` from the ± energies and the cubic constants); sheet 8: the ΔH model's output includes the totally symmetric relaxation and a `Geometry term` step sits between the band positions and the profile. The 28 September table shows four columns per mode — curvature, geometry term, sum, literature CCSD(T) — all twenty modes, the two open b2g rows flagged (the user's second yes). Details: `probes/results_m1/R0_DIAGONAL_READING_2026-09-22.md`.

**Decision 49 (2026-09-23 — the couplings are learned in local coordinates, not in the mode basis; the user: "Goed idee, onthoud het", then "Doe wat nodig is om aan de supervisor aan te tonen dat ons idee werkt"):** the learned correction of the force field is predicted as pairwise local terms in primitive internal coordinates (diagonal, pairs sharing an atom, bond–bond pairs inside a ring) and projected onto whatever mode basis the pipeline needs; the mode-basis block target of module 05 is kept as that module's pre-registered baseline and is not carried further for couplings. Evidence of the day (pre-registered E6 and E7): mode-basis models sit on the zero rule for couplings at 45, 100 and 175 molecules (slopes 0.00 to −0.04); the parameter-free projection shows three quarters of ΔH on the pairwise pattern; the pairwise model reaches a ring coupling ratio of 0.43 / 0.47 on the two hold-outs with corrected-frequency errors of 4.7 / 5.1 cm⁻¹ against 23 for no correction. Read-outs are basis-free from now on (corrected frequencies after diagonalisation, Duschinsky overlap). Whether the coupled-cluster correction lives in the same pattern is E8 (benzene running on hel1-16; naphthalene pre-authorised on a win). Blog post 11; demonstration note `GoalGathering/notes/Demonstration_2026-09-23_Couplings_Local_Representation.md`.

**Decision 50 (2026-09-23 — data quality of the corpus: every Hessian gets a second route when flagged; the user: "laten we onze softwarekwaliteit in de gaten blijven houden"):** benzene's corpus ωB97X finite-difference Hessian was wrong by up to 133 cm⁻¹ (psi4's default 75/302 grid with a 0.005 bohr step, reproduced exactly; 99/590 removes it). Guards: the sorted-pair functional-shift screen in `corpus/check_results.py` (> 80 cm⁻¹ flags), the analytic second route `corpus/analytic_hessians.py`, `build_release.py --prefer-analytic`; new corpus layers use analytic Hessians or grid 99/590, and molecules above C2v get the second route by default. Layers A and A2 stay as computed (screened; benzene replaced). Module 05 adopts the correction as follow-up cells (section 7), not as replacements, so the reviewer sees what was learned.
...
3. the corpus factory's five-molecule timing test (`modules/05_support_predictor/corpus/run_corpus.py --max-molecules 5
   --grid-check`) — **running since 2026-09-14 08:35** (a first launch at 08:25 as a PowerShell job died with its session; relaunched in a persistent background shell with the qc environment's `Libraryin` on PATH; log `corpus/timing_test_2026-09-14.log`), then the dated note fixing the subset size;
3b. **pyVPT2 installation (permitted by the user 2026-09-14 10:2x, after the timing test) — in progress 16:2x–16:4x:** installed into `qc` (0.1.2, 32 KB) but not importable there — pyVPT2 (0.1.2 and `main`) uses pydantic-1-era qcelemental API (`ProtoModel.Config`, `models.molecule.GEOMETRY_NOISE`) that qcelemental 0.51 dropped, and a driver-side shim breaks psi4 1.11's own import; so a **separate environment `vpt2`** (pyVPT2 0.1.2, qcelemental 0.30.1, qcengine 0.34.2, psi4 1.10.2, 116 MB) was created, whose psi4 first failed on a LibXC mismatch (being pinned); mandate ledger log 16:2x; Software Changes Ledger row when it runs; the anharmonic step then runs on psi4 1.10.2 (a version difference to the corpus's 1.11, recorded). Original item: `pyvpt2` 0.1.2 from conda-forge into the qc environment (all run dependencies already present; `--freeze-installed`), Software Changes Ledger row the same day, benzene B3LYP/6-31G* self-test; purpose: the Δ₂ = 0 null column's anharmonic positions for the shape score (mandate ledger obstacle 17) and the base for idea I6 (VPT2 intensities);
4b. **M3 — DZ-anchored decks (pre-registered 2026-09-14, `notes/PreRegistration_2026-09-14_M3_DZ_Anchored_Decks.md`; mandate ledger I7) — first after M2a by decision 38 (DZ cells, then the gap for item 5 / pyVPT2 benzene / the app update, then the TZ cells):** naphthalene, one mode per judged family, 5 points in cc-pVDZ and cc-pVTZ at tight; ≈ 13 laptop-hours + ≈ 6 laptop-days; after M2a; the user chooses M3 or the σ-run (decision 35) first; win/lose fixed in the note;
   *Status 15 September 14:0x:* **DZ cells complete** (14 Sep 17:51 → 15 Sep 13:43; 69 min per energy measured, ratio 9.9–10.0 to TZ tight, 1.7 GB; `results_m1/naphthalene_cc-pvdz_tight_m3/`, reading `results_m1/M3_EVEN_ODD_READING_2026-09-15.md`); the DZ curvatures and beyond-MP2 increments per mode are in hand, **the verdict waits for the TZ cells** (same three modes, tonight ≈ 20:30, ≈ 6 laptop-days; q = 0 computed once by this morning's patch). Side finding: the factory modes' irrep impurity (1e-4) explains the odd parts of modes 12 and 31 at the component level (`results_dryrun/naphthalene/MODE_PURITY_2026-09-15.md`).
4. ~~plan 06's X6 and X7~~ — **both run 2026-09-14 08:18–08:24**: X6 lost (π-CAS share 0.09 on the C–C stretch; S3 closed), X7 not lost, not confirmed (λ·gap max/min 1.57; two script fixes on the way, pair sum = E_corr to 10⁻¹⁵); plan 06 notes X6/X7 of 14 September;
   ...
   4a. **M2a — the gradient-to-energy cost ratio g** (pre-registered 13 September, `notes/PreRegistration_2026-09-13_M2a_Gradient_Cost_Ratio.md`; script `probes/m2a_gradient_cost_ratio.py`): install PySCFAD 0.3.3 in `~/qcad` (download — the user's permission first), then cells 0–4 at benzene cc-pVDZ (an evening), cell 5 at cc-pVTZ only if cell 3 gives g ≤ 20; **run 2026-09-14 16:28–17:50 (`probes/results_m2a/m2a_cc-pvdz.md`): g = 2.84 (RHF) and 3.20 (MP2); cells 2–4 (canonical CCSD(T), LNO-CCSD(T)) do not fit the 25 GB VM — the PySCFAD energy step alone exceeds it (21 GB RSS + 6.8 GB swap; host < 1 GB free twice) — so g at the anchor's level is unmeasurable on the laptop; plan-06 decision rule §2 row updated; memory-lean routes recorded (checkpointed LNO; a 64–128 GB machine), not scheduled;**
...
**Shape score pre-registered (14 September):** `GoalGathering/notes/PreRegistration_2026-09-14_Shape_Score_Cold_Spectra.md` — how a predicted spectrum is scored on *shape* against the jet-cooled band lists of tetracene, coronene, peropyrene, ovalene and HBC (10 cm⁻¹ bins, earth mover's distance per family, family-weight error; columns A, B, D, null, pipeline, network; losing and winning conditions fixed before any prediction exists) — the evaluation the user's success criterion needs on R2–R3-size molecules where positions are not decidable.

**Ladder for the supervisor (14 September):** `GoalGathering/Frozen_Ladder_and_Tolerances_2026-09-14.md` — the ladder written once, as one document: sentence types, rungs with their named laboratory columns (incl. the naphthalene band-origin column, items 72–73), the decidability rule with the temperature term per source class, the rules fixed now (K, frozen spaces with the composite and basis terms, symmetry prior, fragment licence, Q6/Q7/Q8), the fifteen pilot-note items, the stop conditions and the refusals; no dated notes, no review history. The working record with its measurement log stays `Frozen_Ladder_and_Tolerances.md`.

**Compute budget for the supervisor (14 September):** `GoalGathering/Compute_Budget_2026-09-14.md` — the budget written once, as one document: the three budgets, the classification rule, every measured timing with its script, the anchor thresholds and the F slot (NOT_RUN until the running timing prints), the mode-E planning count, the Snellius orientation column, the priced items, the operating rules for long runs, and the timed probes done and remaining. The working record with its measurement log stays `Compute_Budget_2026-09-03.md`.

**The user's mandate (13 September, evening; `notes/Mandate_2026-09-13_Affordable_Plan_Obstacle_Ledger.md`):** an affordable plan — the new desktop plus a small Snellius request — that by 2027 yields a trained network in a pipeline "large PAH in, spectral shape out", better than the existing predictions on R4–R6 or with a measured, smaller error budget; the note holds the goal sentence, what the budget buys in labels per year (P26 block: one or two full naphthalene-class corrections, about ten diagonal-only, a hundred benzene-class, one or two thin pyrene-class checks per year at provisional F), and a ledger of ten obstacles with the best solution so far, its status and the deciding test, appended whenever one moves; standing instruction to keep working the cheapest untested obstacle whenever the machine is busy or the user is away.
...
**Duration note (13 September, `notes/Duration_Note_2026-09-13.md`, table printed by `probes/duration_table.py`):** every computation through Module 08 per route (laptop / desktop / Snellius), measured or labelled; **xtight factor F = 3.34 measured 14 September 07:49** (38.4 h of fragment solves against 11.5 h at tight; the out-of-core path is in the ratio) — R1 deck 759 laptop-days, 198–330 desktop-days (estimate), 25–41 days on four Snellius nodes (estimate); R0–R1 within reach, **R2–R3 as full decks out of reach on every route** (the decks, not the machines); one open place (g from M2a); five questions to the user before it enters proposal §12.
