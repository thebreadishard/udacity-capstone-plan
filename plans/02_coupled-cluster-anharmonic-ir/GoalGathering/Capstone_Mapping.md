...
| **1 — H₂O PES training** | Learn R → E, F, ρ | ≥2,000 H₂O configs from the §5.1 PySCF campaign (same-surface CCSD(T)/cc-pVTZ energy and complete gradients; \(\rho\)/\(H\) per the pinned recipe) | Supervised regression; custom energy-based hybrid FNO + local-conv ("NCA") architecture | Trained PES model, force RMSE / harmonic-frequency validation | **Workstream P1** (ungraded; §4.1) — *not* Modules 04/05/07 |
| **2 — Frozen-weight IR (H₂O)** | Spectral prediction with no spectral fitting | 5×50 ps MD trajectories (simulation output, not a training dataset) | Inference-only / simulation after static \(E/F/H/\rho/\mu\) training | FFT-derived IR envelope vs. experimental FTIR | Module 07 (tool-use demo), on **P1's frozen weights** |
...
| 04 | H₂O **descriptor CSV** from the Distilled Plan §5.1 campaign (same geometries as P1; not the volumetric tensors) | CSV, one row per config: `config_id, r_OH1, r_OH2, theta_HOH, energy_hartree, fx_O, fy_O, fz_O, fx_H1, …` plus the §5.1 same-surface derivative tags (`derivative_kind`, `derivative_theory`, `fd_step_bohr`, `derivative_uncertainty`, `pyscf_version`, …). H₂O uses complete CCSD(T)-surface gradients. Bond-length/angle descriptor → energy/forces; deliberately *not* the 3D density grid. CSV **copy in the submission folder** (`df.head`). | `modeling.ipynb`, `Machine_Learning_Analysis_Report.pdf`, `requirements.txt`, dataset CSV | H₂O campaign per Distilled Plan §5.1. Same geometries may also feed P1; only this CSV is the Module 04 dataset. **§5.5:** Zenodo DOI **before** the source sentence. |
| 05 | Benzene configuration set (Phase 5), **nominal** ≥5,000 configs, 64³ grid — **per Distilled Plan §5.1** | Volumetric tensors (`.npz`/HDF5), one file per config: nuclear positions, target ρ(r) and analytic AO dipole from the pinned 1-RDM recipe, CCSD(T) energy, and either a complete same-surface gradient or seeded directional derivative; selected Hessian entries — plus an indexing manifest CSV with derivative and dipole provenance; **not CSV-only**; **not** the 04 CSV and **not** P1 tensors | `deep_learning.ipynb`, `Deep_Learning_Systems_Analysis_Report.pdf`, `requirements.txt`, dataset **hosted externally with access instructions** | Phase 5 benzene campaign — **the single biggest compute bottleneck**. \(N\), grid and derivative rung are **targets until the 10-geometry pilot exits Phase 0**. If the same-surface derivative ladder or shrink ladder reaches remapping, 05 must be remapped. Prefer P1 architecture-stable before 05 training starts (§4.1). **§5.5:** Zenodo DOI **before** the source sentence. |
...
**Decision (2026-08-22):** Phase 1 is not crammed into Modules 04, 05, or 07. It is an explicit **ungraded research workstream** with the same seriousness as a module row. 04 stays the tabular non-field baseline; 05 stays the graded CNN-family benzene scale-up; 07 stays a tool-user over logged results. See [Professor_Review_2026-08-22_Round1.md](../../01_voxel-field-pes/GoalGathering/Professor_Review_2026-08-22_Round1.md) blocking issue 1.
...
| **What it trains** | Distilled Plan §6 forward pass: Route B \(E=\mathcal{E}[\rho,R]\) (fixed Hockney–Eastwood \(E_{\mathrm{es}}\) + learned \(\varepsilon_\theta[\rho]\); NCA ± FNO encoder) on H₂O volumetric fields, \(32^3\), \(\ge 2000\) configs **if the H₂O pilot allows**. No latent energy head. It trains the full \(E/F/H/\rho\) production model plus the §7.1 comparison legs **Field-EF** and **Field-EFρ**. Density target is the §5.1 1-RDM recipe, not “exact CCSD(T) density.” |
| **Deliverables** | Frozen production, Field-EF and Field-EFρ weights; force/energy/harmonic-frequency and dipole-gate report vs Distilled Plan §7 Phase 1 gates; config-level split and observable manifests |
...
Same §5.1 geometries may feed 04 (bond/angle + \(E,F\)) and P1 (\(\rho, E, F, H\)); H₂O uses complete same-surface gradients. That is **one data campaign, two products**. Only the CSV is the Module 04 dataset. Recipe, cost pilot, and shrink ladder live in Distilled Plan §5.1, not in this table.
...
| **Inputs** | P1 H₂O **split manifest** and, if 05 runs, the 05 benzene **split manifest**. Geometry, CCSD(T) energy and the accepted same-surface complete/directional derivative labels only. Same `config_id`s and direction seeds. **No new sampling.** |
...
| **Primary metric (the §2 test)** | Leave-one-mode-out (or held-out mode-family) force error for **Field-EF vs MACE-EF**, trained on identical \(E/F\) labels and the same split — H₂O first; benzene if 05 exists. **Field-EFρ vs Field-EF** is the secondary density-supervision ablation. The full production model cannot establish a representation-only win. |
| **Pre-registration** | Distilled Plan **§7.1** is a precondition, not a courtesy: frozen split file + hash committed **before** any leg trains, \(\ge3\) seeds with error bars, equal hyperparameter budget (MACE starts from its authors' recipe — an untuned competitor is a straw man), a declared effect size \(\Delta\), and “inconclusive” pre-authorised as a publishable outcome. Test set touched once. |
...
|---|---|---|
| Train/Validation/Test sets use same-surface **CCSD(T)/cc-pVTZ-level labels**; “chemically accurate” is conditional on the Distilled Plan §5.1 CBS(T,Q) audit. Sub-cm⁻¹ is a **spectrum** claim and is forbidden for this thesis (Distilled Plan §9; Overarching Goal §3). | 04 (H₂O descriptors), **P1** (H₂O volumetric field PES), **G1** (same energy/derivative IDs), 05 (benzene) — the actual pipeline's ML data | **Compliant on method and provenance; precision verdict pending the audit.** Energies and supervised derivatives come from the same CCSD(T)/cc-pVTZ surface. H₂O uses complete gradients; benzene may use seeded directional derivatives if the measured pilot rejects complete gradients. CCSD forces are diagnostics only, never targets. The frozen CBS(T,Q) audit determines molecule- and quantity-specific precision wording. Density is the pinned 1-RDM recipe (default: relaxed CCSD), a documented density-level gap. A cheaper density proxy is **not** compliant unless the §5.1 shrink ladder fires and the Overarching Goal escape clause is invoked. |
...
| NOT DFT, no exceptions for pipeline data | 04, P1, 05 (the real pipeline) | **Compliant by default** — energies and supervised derivatives belong to the same CCSD(T) surface. DFT/HF-level data appears *only* in 02 and 06, plus **rung 3 of the §5.1 shrink ladder** (density proxy only), which is a real exception and must invoke the escape clause. Hohenberg–Kohn *shape* \(E=\mathcal{E}[\rho]\) is the claim, not KS-DFT / library XC. |
...
| NOT trained on spectral loss | P1, 05 | **Action item:** the production loss is the pre-registered static-label objective $L_E+L_F/L_D+L_H+L_\rho+L_\mu$; comparison legs exclude $L_\mu$ per Distilled Plan §6.3. Any MD/FFT spectrum is frozen-weight post-hoc evaluation. Spectra, peak positions, intensities and dipole derivatives are never training signals. |
...
| NOT treating supercomputer time as unmeasured production capacity | 0b, 05 | **Compliant with a narrow exception:** limited HPC is authorized up front for the frozen §5.1 CCSD(T)/CBS(T,Q) label audit, after a one-geometry benzene QZ resource pilot confirms the allocation and calendar. Architecture development remains local. Module 05 production data still follows its own measured pilot and shrink ladder; audit allocation is not silently repurposed. |
...
| **04** | H₂O descriptor CSV | Zenodo DOI **before** Task 1; CSV copy in the zip for `df.head`. | Computed with **PySCF** under Distilled Plan §5.1 (ab initio CCSD(T)/cc-pVTZ energy and complete same-surface gradients) — first-principles numerical data, **not** an AI/ML-generated synthetic dataset. |
