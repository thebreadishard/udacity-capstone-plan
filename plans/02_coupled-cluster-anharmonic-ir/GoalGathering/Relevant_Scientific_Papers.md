...
16. **Infrared Spectroscopy of Matrix Isolated Polycyclic Aromatic Hydrocarbons. 1. PAHs Containing Two to Four Rings** (Hudgins & Sandford, *J. Phys. Chem. A* **102**(2), 329–343, 1998)
    ...
    - *Significance (added 2026-08-27):* The title's scope covers the C₁₈H₁₂ four-ring isomers used in this repository's controlled bay series. **The band values themselves remain UNVERIFIED**: tetracene ≈ 742.9, triphenylene ≈ 740.8 and chrysene ≈ 744–748 cm⁻¹ (argon matrix) were taken from a third-hand summary in [grok_chat_3.md](../../../AI_Chats/grok_chat_3.md), not read from the paper. If they hold, the measured quartet band barely moves with bay count, whereas B3LYP/6-31G* puts the same three at 735.0, 733.8 and 751.8 cm⁻¹ — meaning the +16.8 cm⁻¹ chrysene step is a property of the functional and the repository's bay conclusion is right for a partly wrong reason.
    - *Alternative route to the same numbers, no paywall:* the NASA Ames PAH IR Spectroscopic Database (item 18, Boersma et al. 2014) carries an experimental library with the source reference attached to each spectrum. Checking there is equivalent evidence and is available now.
    - **Until one of those two checks is done, no sentence in this repository may rest on these numbers.**
