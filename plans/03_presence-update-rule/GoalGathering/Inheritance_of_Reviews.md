...
**Open in 03** is a flag on a subset, not a fifth fate: **3** (no teacher cube), **5 / R3-3** (voxel DOI + mentor approval; bibliography item 10 FAIL), **7** (Q1/Q2 on a real teacher), **14** (M03 flatten not printed), **15 / R3-6** (T0 not a calendar date), **R4B-2** (`teacher_cost.py` prints `NOT_RUN`).

Closing a row in *spec* is not a green light. Plan 03 has zero professor reviews of its own.
...
| 2 | Energy is not yet a functional of the field | **Superseded** as a PES. Plan 03 does not learn \(E[\rho]\). Distilled §5 still requires an implementable 12-channel conv, not a slogan. | Distilled §5 |
...
| 5 | Rubric landmines (synthetic / accepted sources / CNN-family) | **Carried.** M02–M04 third-party tables; M04 Kaggle/UCI/Data.gov only; M05 CNN-family explicit. **Open:** M05 Task A voxel DOI unpinned (bibliography item 10 FAIL). | Mapping; [Relevant_Scientific_Papers.md](Relevant_Scientific_Papers.md) |
...
| 14 | Module 03 row-count arithmetic / noise model | **Superseded** as that egg-box factorial. M03 is a flatten of RODARE 3995 (or QM7-X fallback). **Open in 03** until the flatten script exists and row counts are printed, not typed. | Mapping M03; bibliography item 3 |
...
| R3-3 | Modules 04–06 dataset eligibility / synthetic | **Carried, still open.** Same mentor-approval hole. Self-run Octopus cubes are **computational experiments** (Task B), not the graded Task A set. Task A still needs a public voxel DOI or the pre-declared fallback. | Mapping M04–M06; bibliography item 10 FAIL |
...
| # | Finding | Fate in 03 | Where |
|---|---|---|---|
| R4A-1 | Stale README / reading-order vs rewrite-status | **Carried as a class.** Plan 03 README already says **draft, not complete.** [PATCH_plans_README.md](../PATCH_plans_README.md) must **not** say “complete as a plan” until a review closes. | [README.md](../README.md) |
| R4A-2 | Adopted-vs-draft status drift | **Carried.** This folder is **draft as of 2026-09-01.** Plans 01/02 remain until a deletion pass. | README status banner |
...
Plan 03 has **zero** professor reviews. The briefs ask for a **new** Round 5 (Pass A cold, Pass B adversarial TDDFT / ML-propagator), not a copy of 01 or 02.

- [Review_Brief_2026-09-01_Round5_PassA.md](Review_Brief_2026-09-01_Round5_PassA.md)
- [Review_Brief_2026-09-01_Round5_PassB.md](Review_Brief_2026-09-01_Round5_PassB.md) — give only after Pass A is written down.
