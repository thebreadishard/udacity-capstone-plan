...
Combinations that are additive: A × B (thresholds and element count are independent) → 474 → ≈ 300 energies at 2.1–4.7× lower price: **the deck in ≈ 1.5–5 months on the desktop, ≈ 1 week on four Snellius nodes** (*e*, from the P13 rows). B does not stack with C in the obvious way (substitution reads the whole pattern; a smaller pattern lowers the colour count somewhat, X1c's maxr would have to be recounted). C or D with a measured small g is the only lever that changes the *order of magnitude*, and it is the only one that also grows with size (X8 brackets).
