...
- **X7 (S1, defined 2026-09-12 evening, not run — compute):** decay length λ of the pair-energy (or atom-pair block) profile against the DFT HOMO–LUMO gap for benzene, naphthalene, pyrene; losing condition for "rate ∝ gap": λ·gap varies by more than a factor two. **Script written 2026-09-12 evening (`experiments/x7_decay_vs_gap.py`, not run):** DF-MP2/cc-pVDZ pair energies rotated exactly into Pipek–Mezey LMOs, λ by X3b's shell fit, B3LYP/6-31G* and RHF gaps at the same geometry; benzene and naphthalene from the dry-run geometries, pyrene from plan 02's stored geometry; geometry self-test passed; PySCF API checked in WSL; runs after the anchor job (minutes). **Run 2026-09-14 (after two fixes: conventional RMP2 integrals; pair energies formed in the localised basis, pair sum = E_corr to 1e-15): λ·gap max/min 1.57, not lost, not confirmed.**
- **X6 (S3, defined 2026-09-12 evening, not run — compute):** finite-difference curvature of E_CASCI(6,6) − E_HF along the three probed benzene modes against the sealed CC − DFT curvature; losing condition: π-CAS share < ½ on the C–C stretch mode closes S3 as an anchor route. **Script written the same evening (`experiments/x6_pi_cas_share.py`, not run):** to keep plan 05's seal it compares the π-CAS(6,6) curvature (AVAS on C 2pz) with the *MP2* correlation curvature along M1's three modes and q grid at cc-pVDZ; the sealed CC comparison waits for the pilot note; geometry self-test passed on Windows (numpy only); runs in WSL after the anchor job. **Run 2026-09-14 08:18 (cc-pVDZ, 9 points, three modes): LOST — share 0.09 on the C–C stretch, −0.48 on C–H oop, the in-plane bend's fit not trustworthy; S3 closed.**
...
- **X16 (test battery; written 2026-09-13, self-test passed):** `experiments/x16_tensor_tests.py --molecule <m>` runs X10 (P25's licence test, verdict printed against the pre-stated conditions), X11, X9 and X8 on any dry-run stage-A tensor; on benzene it reproduces X10 (19/17/6), X11 (4/3/2), X9 (0.0268/0.0647) and X8 (20 blocks, 41.69 cm⁻¹) exactly; irreps from the dry run's prior file or a D2h analysis of the modes. Waits for naphthalene's stage A.
- **X15 (S1/T3; done 2026-09-13 on plan 02's Hessians — [result note](Result_Note_2026-09-13_X15_ODLR_Far_Blocks.md)): ODLR holds in norm, fails at 0.5 cm⁻¹** — the far blocks of eight PAH Hessians need r\* = 9–30 of 24–36 singular directions (r\*/dim 0.30–0.83; only chrysene reaches the pre-stated third), even for acene end rings 9 bohr apart; T3's ODLR form is a norm-level conjecture only; lever C beyond the symmetry prior not licensed at tolerance. Definition: singular values of the off-diagonal blocks between two distant atom groups (end rings of tetracene, opposite sectors of coronene) of the mass-weighted B3LYP Hessian, and the rank r at which replacing the far blocks by their rank-r truncation keeps every band within 0.5 cm⁻¹; losing condition for ODLR at plan 05's tolerance: r above a third of the block dimension ([reading note](Reading_Note_2026-09-13_O1NumHess.md) §3).
- **X14 (S5; done 2026-09-13 morning):** substitution products on naphthalene's real symmetry pattern from X13 — **9 products = 18 gradients recover the whole R1 correction against plan 05's 474-energy deck, g\* = 26**; k equals the largest irrep block (Ag, 9 modes), so products grow ∝ M/|G| while the energy deck grows ∝ M²/|G| ([result note](Result_Note_2026-09-13_X14_Naphthalene_Pattern_Products.md)). M2a's bar at R1 is set before M2a runs.
- **X13 (S4; done 2026-09-12 evening):** the naphthalene DFT mode table from plan 02's stored B3LYP/6-31G* Hessian — 48 modes with D2h irreps and the dry run's family labels, **141 same-irrep pairs of 1,128 (12.5 %)**, which is exactly the 282 off-diagonal energies of plan 05's R1 deck at 2 per pair (proposal §3.2: an independent cross-check); the DFT-only denominator ranking over those pairs pre-tabulated so that the naphthalene repeat of X10 (draft P25's licence test) reads a table it did not choose ([result note](Result_Note_2026-09-12_X13_Naphthalene_Mode_Table.md), `experiments/x13_naphthalene_mode_table.py`).
...
- Wang, B., Luo, S., Wang, Z., & Liu, W. (2025). O1NumHess: A fast and accurate seminumerical Hessian algorithm using only O(1) gradients. *J. Chem. Theory Comput., 21*(21), 10893–10909. https://doi.org/10.1021/acs.jctc.5c01354 (read in full 2026-09-13, arXiv:2508.07544)
- Sanders, J. N., Andrade, X., & Aspuru-Guzik, A. (2015). Compressed sensing for the fast computation of matrices: Application to molecular vibrations. *ACS Cent. Sci., 1*(1), 24–32. https://doi.org/10.1021/oc5000404 (record verified 2026-09-13; plan 05 item 24)
- Yang, X., Ma, H., Lu, Q., & Bian, W. (2024). Efficient method for numerical calculations of molecular vibrational frequencies by exploiting sparseness of Hessian matrix. *J. Phys. Chem. A, 128*(15), 3024–3032. https://doi.org/10.1021/acs.jpca.3c07645 (record verified 2026-09-13)
- Rai, P., Sargsyan, K., Najm, H., & Hirata, S. (2019). Sparse low rank approximation of potential energy surfaces with applications in estimation of anharmonic zero point energies and frequencies. *J. Math. Chem., 57*, 1732–1754. https://doi.org/10.1007/s10910-019-01034-z (record verified 2026-09-13)
- Reiher, M., & Neugebauer, J. (2003). A mode-selective quantum chemical method for tracking molecular vibrations applied to functionalized carbon nanotubes. *J. Chem. Phys., 118*(4), 1634–1641. https://doi.org/10.1063/1.1523908 (record verified 2026-09-13)
