...
| S5 query algebra (secondary track) | E1 | **alive, conditional** (2026-09-12, X1 done): 53 of 435 elements above 1 µE_h; exact sparse recovery needs 4–5 Hessian–vector products (greedy colouring bound) = 240–300 energies if a product costs 2M energies, against K = 448; rank is full (30), no low-rank route | X1 done; Crossref check the same day: analytic gradients exist for DLPNO-MP2 (Pinski & Neese 2018, 2019), none found for local CCSD(T) → **parked** | revives only on a verified local-CC(T) gradient record |
