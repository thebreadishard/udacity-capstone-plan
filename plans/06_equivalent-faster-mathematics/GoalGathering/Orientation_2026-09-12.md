...
- **X9 (S1, T3′; defined 2026-09-12 evening, not run — minutes, numpy):** atom-pair block profiles of the mass-weighted DFT Hessian and of the stand-in Δ₂ side by side on the stored benzene npz, by bond-graph distance and by shell, at norm level and at band level (X2's rule); losing condition for T3′ at benzene: the ratio ‖Δ₂[A,B]‖/‖H_DFT[A,B]‖ does not fall with distance.
