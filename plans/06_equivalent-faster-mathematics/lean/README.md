...
**Build 2026-09-14 08:0x–08:2x (Windows, between anchor jobs; host 19.6 GB free).** `lake build` of the root module: 8,766 jobs, 139 s (Mathlib oleans from the cache). `Plan06.T1.SymmetricColouring` (T1d) failed on the first attempt at lines 51–52 — the two `exact` lines of the properness half handed the readability hypothesis five arguments where its type (`∀ j', (i, j') ∈ P → j' ≠ j → c j' ≠ c j`) takes four; corrected to `hr i (hdiag i) hne hcij` and `hr j (hdiag j) hne.symm hcij.symm`, and the deprecated `push_neg` replaced by `push Not`. **Second attempt: T1d builds (36 s), no error, no `sorry`; imported into `Plan06.lean`; the root module rebuilds in 35 s (8,767 jobs).** Coleman & Moré's Theorem 2.2 — symmetric validity ⇔ proper colouring of the pattern graph with no 2-coloured path of length 3, for a symmetric pattern containing the diagonal — is now machine-checked in both directions (`symmValid_iff`). Remaining lint: long-line warnings only.
