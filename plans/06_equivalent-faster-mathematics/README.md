...
11. [lean/](lean/README.md) — the formal route: a Lake project on Mathlib `v4.34.0-rc2` (installed 2026-09-12); **built 2026-09-14: root module OK; T1d (Coleman & Moré Theorem 2.2, `symmValid_iff`) builds after a two-line fix, no `sorry`, and is imported into the root — the symmetric-colouring characterisation is now machine-checked;** `Plan06/T1/MeasurementAlgebra.lean` holds T1 — T1a, T1b and T1c all proved (no `sorry`): exact recovery from colour-class probes, Δ + 1 probes suffice, and the symmetric one-sided scheme. Build with `lake exe cache get && lake build`; `.lake/` is git-ignored.
...
20. [GoalGathering/Result_Note_2026-09-13_X14_Naphthalene_Pattern_Products.md](GoalGathering/Result_Note_2026-09-13_X14_Naphthalene_Pattern_Products.md) — X14: on naphthalene's real symmetry pattern 9 substitution products = 18 gradients replace the 474-energy R1 deck (g* = 26); products scale with the largest irrep block, the deck with the pair count.
21. [GoalGathering/Decision_Rule_2026-09-13_When_Plan06_Closes_or_Transfers.md](GoalGathering/Decision_Rule_2026-09-13_When_Plan06_Closes_or_Transfers.md) — decision rule (13 September, **adopted**): the pending numbers and what each does; branch C closes by 15 October if g > 26 and P25 is not licensed; branch M reviewed 1 December; the Lean route has no closing date.
...
27. [GoalGathering/Reading_Note_2026-09-13_Literature_Check_Leads_A-G.md](GoalGathering/Reading_Note_2026-09-13_Literature_Check_Leads_A-G.md) — literature check (OpenAlex/Crossref, abstract level, every record DOI-verified) for the reflection's leads A–G: nearest prior art for the whole project is the Δ-ML/transfer-learning line (Käser 2021; Qu, Bowman 2021), not PAH work; lead A's tool exists (OT-RSH) but not its use, and the literature predicts its failure mode (ω drifts with acene length); the SQM lineage is lead D's ancestor; two rotationally resolved naphthalene ν46 band origins (Albert 2011, Pirali 2013) are missing from Module 03; scripts `experiments/lit_search_openalex.py`, `experiments/lit_search_targeted.py`.
28. [GoalGathering/X17_2026-09-13_Internal_Coordinate_Orbits.md](GoalGathering/X17_2026-09-13_Internal_Coordinate_Orbits.md) — lead D's first desk test (benzene stand-in): the correction written in symmetry orbits of 42 redundant internal coordinates is not compact (diagonal only 18.5 cm⁻¹ RMS, shared-atom couplings 6.3) but terminates exactly one bond out (one-bond-apart couplings 0.03 cm⁻¹, nothing beyond); C–H stretch corrections are σ-internal, the C–H out-of-plane family cannot be σ/π-split (25 cm⁻¹ without cross couplings); the real test is type transfer to naphthalene after machine queue item 5. `experiments/x17_internal_coordinate_orbits.py`.
29. `experiments/x18_type_transfer.py` — prepared 13 September evening: type transfer of the internal-coordinate correction (X17's S2 reach, one constant per local pair type, in-plane block only because out-of-plane signs do not transfer by type), source-fitted and read per family in the T-1 form; self-test benzene→benzene reproduces the in-plane families to 0.01–0.05 cm⁻¹ with 29 types; benzene→naphthalene NOT_RUN until plan 05 machine-queue item 5.
