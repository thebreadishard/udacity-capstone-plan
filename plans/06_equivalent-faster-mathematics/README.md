...
11. [lean/](lean/README.md) — the formal route: a Lake project on Mathlib `v4.34.0-rc2` (installed 2026-09-12); **built 2026-09-14: root module OK (8,766 jobs, 139 s); T1d fails at lines 51–52 (type mismatch) and stays a draft;** `Plan06/T1/MeasurementAlgebra.lean` holds T1 — T1a, T1b and T1c all proved (no `sorry`): exact recovery from colour-class probes, Δ + 1 probes suffice, and the symmetric one-sided scheme. Build with `lake exe cache get && lake build`; `.lake/` is git-ignored.
