...
|---|---|---|
| Train/Validation/Test sets must be chemically precise **labels** (CCSD(T)/cc-pVTZ per Distilled Plan §5.1). Sub-cm⁻¹ is a **spectrum** claim and is forbidden for this thesis (Distilled Plan §9; Overarching Goal §3). | 04 (H₂O descriptors), **P1** (H₂O volumetric field PES), **G1** (same energy/derivative IDs), 05 (benzene) — the actual pipeline's ML data | **Compliant on energy and supervised derivatives** — both come from the same CCSD(T)/cc-pVTZ surface per Distilled Plan §5.1. H₂O uses complete gradients; benzene may use seeded directional derivatives if the measured pilot rejects complete gradients. CCSD forces are diagnostics only, never targets. Density is the pinned 1-RDM recipe (default: relaxed CCSD), a documented density-level gap. A cheaper density proxy is **not** compliant unless the §5.1 shrink ladder fires and the Overarching Goal escape clause is invoked. P1 and G1 are ungraded infrastructure, not extra graded datasets. |
