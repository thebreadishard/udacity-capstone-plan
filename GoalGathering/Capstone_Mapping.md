...
| 05 | Benzene configuration set (Phase 5), **nominal** ≥5,000 configs, 64³ grid — **per Distilled Plan §5.1** | Volumetric tensors (`.npz`/HDF5), one file per config: nuclear positions, target ρ(r) from the pinned 1-RDM recipe, CCSD(T) energy, and either a complete same-surface gradient or seeded directional derivative; selected Hessian entries — plus an indexing manifest CSV with the §5.1 derivative provenance; **not CSV-only**; **not** the 04 CSV and **not** P1 tensors | `deep_learning.ipynb`, `Deep_Learning_Systems_Analysis_Report.pdf`, `requirements.txt`, dataset **hosted externally with access instructions** | Phase 5 benzene campaign — **the single biggest compute bottleneck**. \(N\), grid and derivative rung are **targets until the 10-geometry pilot exits Phase 0**. If the same-surface derivative ladder or shrink ladder reaches remapping, 05 must be remapped. Prefer P1 architecture-stable before 05 training starts (§4.1). **§5.5:** Zenodo DOI **before** the source sentence. |
...
| **Deliverables** | Frozen production, Field-EF and Field-EFρ weights; force/energy/harmonic-frequency report vs Distilled Plan §7 Phase 1 gates; config-level split manifest |
