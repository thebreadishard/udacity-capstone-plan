...
- **Rigorously phased**, with hard numerical Go/No-Go gates between phases (§7), scoped to what is achievable in a ~6–7 month master's thesis. Architecture development remains local-first. Limited supercomputer access is explicitly authorized for the frozen §5.1 higher-level label audit; production campaigns require their own measured pilot and may not silently consume that allocation.
